(function(){
  ...
  style.id='spx-encoder-upload-style-v4';
  style.textContent=`
    ...
    .spx-queue{margin-top:11px;border:1px solid #34475a;border-radius:8px;overflow:hidden;background:#101923}.spx-queue-head{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:9px 10px;border-bottom:1px solid #2d3d4d;background:#172331}.spx-queue-head b{font-size:8px;color:#fff}.spx-queue-head span{font-size:7px;color:#8294a8}.spx-queue-list{max-height:208px;overflow:auto}.spx-queue-empty{padding:18px 10px;text-align:center;color:#72859a;font-size:8px;line-height:1.5}.spx-queue-row{display:grid;grid-template-columns:24px minmax(0,1fr) 88px auto 18px;gap:7px;align-items:center;padding:8px 9px;border-bottom:1px solid #293847;cursor:pointer}.spx-queue-row:last-child{border-bottom:0}.spx-queue-row:hover,.spx-queue-row.selected{background:#1b2937}.spx-queue-pos{width:22px;height:22px;border-radius:6px;background:#223247;display:grid;place-items:center;color:#9db0c5;font-size:7px;font-weight:800}.spx-queue-row.next .spx-queue-pos{background:rgba(255,159,67,.16);color:var(--spx-accent)}.spx-queue-file{min-width:0}.spx-queue-file b{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#e6edf5;font-size:8px}.spx-queue-file span{display:block;margin-top:2px;color:#7f91a4;font-size:7px}.spx-task-action{width:88px;border:1px solid #3b4c5d;border-radius:6px;background:#172331;color:#dce6ef;padding:5px 6px;font-size:7px;font-weight:800;cursor:pointer}.spx-task-action.new{color:var(--spx-good);border-color:rgba(90,208,143,.35)}.spx-task-action.update{color:var(--spx-warning);border-color:rgba(255,209,102,.35)}.spx-queue-state{font-size:7px;font-weight:800;color:#9db0c5}.spx-queue-row.next .spx-queue-state{color:var(--spx-accent)}.spx-queue-remove{border:0;background:transparent;color:#72859a;font-size:13px;line-height:1;cursor:pointer;padding:0}.spx-queue-remove:hover{color:#fff}
    ...
    .spx-queue-select{min-width:240px;max-width:350px}.spx-queue-summary{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:7px;margin-top:8px}.spx-queue-summary span{padding:7px 8px;border:1px solid #34475a;border-radius:7px;background:#172331;color:#8192a6;font-size:7px}.spx-queue-summary b{display:block;color:#e8eef5;font-size:11px;margin-bottom:2px}
    @media(max-width:760px){.spx-queue-row{grid-template-columns:24px minmax(0,1fr) 82px 18px}.spx-queue-state{display:none}.spx-queue-summary{grid-template-columns:1fr 1fr}}
  `;
  ...
  function inferredAction(name){return /update|updated|status|picked|pickup|hold|handover|complete|done|retry|change/i.test(String(name||''))?'Update':'New';}

  function readFile(file,index){
    return new Promise(resolve=>{
      if(!file||!file.type?.startsWith('image/')) return resolve(null);
      const reader=new FileReader();
      reader.onload=()=>resolve({id:`q${Date.now()}-${state.seq++}-${index}`,name:file.name,size:file.size,dataUrl:String(reader.result||''),status:'Queued',action:inferredAction(file.name)});
  ...
  async function loadFiles(fileList){
    ...
    renderUpload(true);
    toast(`${items.length} screenshot${items.length===1?'':'s'} added. Set each one as New task or Task update before processing if needed.`);
  }

  function queueMarkup(){
    const selected=currentItem();
    const rows=state.queue.map((item,index)=>`<div class="spx-queue-row ${index===0?'next':''} ${selected?.id===item.id?'selected':''}" data-queue-id="${esc(item.id)}"><span class="spx-queue-pos">${String(index+1).padStart(2,'0')}</span><div class="spx-queue-file"><b>${esc(item.name)}</b><span>${formatBytes(item.size)} · uploaded screenshot</span></div><select class="spx-task-action ${item.action==='Update'?'update':'new'}" data-action-id="${esc(item.id)}" aria-label="Task action for ${esc(item.name)}"><option value="New" ${item.action==='New'?'selected':''}>NEW TASK</option><option value="Update" ${item.action==='Update'?'selected':''}>TASK UPDATE</option></select><span class="spx-queue-state">${index===0?'NEXT':'QUEUED'}</span><button type="button" class="spx-queue-remove" data-remove-id="${esc(item.id)}" aria-label="Remove ${esc(item.name)}">×</button></div>`).join('');
    return `<div class="spx-queue"><div class="spx-queue-head"><b>Unprocessed screenshot tasks</b><span>${state.queue.length} waiting</span></div><div class="spx-queue-list">${rows||'<div class="spx-queue-empty">No unprocessed screenshots.<br>Upload one image or select multiple screenshots at once.</div>'}</div></div>`;
  }

  function uploadMarkup(){
    const selected=currentItem();
    const newCount=state.queue.filter(x=>x.action==='New').length;
    const updateCount=state.queue.filter(x=>x.action==='Update').length;
    return `
      <div class="spx-upload-head"><div><strong>Admin screenshot intake</strong><span>Bulk-upload screenshots for new pickup tasks and updates to existing tasks. They stay in this queue until processed.</span></div><span class="spx-upload-badge">BULK IMAGE INPUT</span></div>
      ...
      ${queueMarkup()}
      <div class="spx-queue-preview ${selected?.dataUrl?'show':''}">${selected?.dataUrl?`<img src="${selected.dataUrl}" alt="Queued SPX screenshot preview"><div class="spx-upload-meta"><b>${esc(selected.name)}</b><span>${selected.action==='Update'?'Task update':'New task'} · ${formatBytes(selected.size)}</span></div>`:''}</div>
      <div class="spx-queue-summary"><span><b>${state.queue.length}</b>waiting</span><span><b>${newCount}</b>new tasks</span><span><b>${updateCount}</b>task updates</span><span><b>${state.completed}</b>processed</span></div>
      <div class="spx-upload-note"><b>Task action:</b> <strong>New task</strong> creates a new normalized Daily Log record after validation. <strong>Task update</strong> applies new pickup/status values to the matching existing task record. The action can be corrected in the queue before processing.</div>
      <div class="spx-upload-note"><b>Queue behavior:</b> the first screenshot is processed next. Processed screenshots leave the waiting queue and the next image moves to the top automatically.</div>
  ...
  function setAction(id,action){
    ...
    if(!item)return;
    item.action=action==='Update'?'Update':'New';
    renderUpload(true);
  }
  ...
  function wireUpload(upload){
    ...
    upload.querySelectorAll('.spx-queue-row').forEach(row=>row.addEventListener('click',e=>{if(e.target.closest('.spx-queue-remove,.spx-task-action'))return;state.selectedId=row.dataset.queueId||'';renderUpload(true)}));
    ...
    upload.querySelectorAll('.spx-task-action').forEach(select=>select.addEventListener('change',e=>{e.stopPropagation();setAction(select.dataset.actionId||'',select.value)}));
  }
  ...
  function mountQueueSelect(){
    ...
    const signature=state.queue.map(x=>`${x.id}:${x.action}`).join('|')+'|'+state.selectedId;
    if(select.dataset.signature!==signature){
      select.dataset.signature=signature;
      select.innerHTML=state.queue.length?state.queue.map((item,index)=>`<option value="${esc(item.id)}" ${state.selectedId===item.id?'selected':''}>${String(index+1).padStart(2,'0')} · ${item.action==='Update'?'UPDATE':'NEW'} · ${esc(item.name)} · ${index===0?'NEXT':'QUEUED'}</option>`).join(''):'<option value="">Queue empty — upload screenshots</option>';
  ...
  function setPendingState(){
    ...
    if(item){
      set(cards[0],item.action==='Update'?'TASK UPDATE':'NEW TASK','queue action');
      set(cards[1],'—','OCR not run yet');
      set(cards[2],'QUEUED','waiting for processor');
      set(cards[3],item.action==='Update'?'Daily Log · update':'Daily Log · new row','after validation');
    }else{
      set(cards[0],'WAITING','upload required');set(cards[1],'—','no screenshot selected');set(cards[2],'QUEUE EMPTY','nothing to process');set(cards[3],'Daily Log','target after validation');
    }
  }
  ...
  function bindProcess(){
    ...
    process.onclick=e=>{
      ...
      toast(item.action==='Update'?`${item.name} processed as a task update and staged against the existing Daily Log record.`:`${item.name} processed as a new task and staged as a new Daily Log record.`);
  ...
  function renderUpload(force){
    ...
    const signature=`${state.queue.map(x=>`${x.id}:${x.name}:${x.size}:${x.action}`).join('|')}|${state.selectedId}|${state.completed}`;
    ...
    if(process){
      process.textContent=next?`Process next · ${next.action==='Update'?'UPDATE':'NEW'} · ${state.queue.length} queued`:'Queue empty';
      process.disabled=!next;
      process.title=next?`Process ${next.name} as ${next.action==='Update'?'an update to an existing task':'a new task'}`:'Upload screenshots before processing';
    }
    bindProcess();
    return true;
  }
