...
function recruitmentV5() {
  ...
  function renderModule(){
    ...
    if(state.module==='Services')return simpleList('Services',['Service','Category','Owner','Status'],[['AI Resume Analysis','Automation','Automation Team','Active'],['Interview Scheduling','Recruitment','Recruitment Ops','Active'],['CRM Migration Support','Data','CRM Admin','Active']],'Create Service');
  ...
  document.querySelectorAll('[data-z5-top]').forEach(b=>b.onclick=()=>{
    ...
    if(tab==='Agents'){setTopActive('Agents');moduleButtons.forEach(x=>x.classList.remove('active'));main.innerHTML=`<div class="z5-module-title"><h1>Agents</h1></div><div class="z5-home"><section><h3>External resume analysis layer</h3><p>n8n is used only to extract resume content, run structured AI analysis against job requirements, validate the result, and write it back to the Zoho CRM applicant record. All other recruitment automation remains inside Zoho CRM workflow rules and Deluge functions.</p><button class="z5-primary" id="z5OpenN8n">Open n8n Resume Analysis</button></section></div>`;document.getElementById('z5OpenN8n').onclick=()=>toast('Open the n8n resume-analysis workflow from the automation demo');return;}
