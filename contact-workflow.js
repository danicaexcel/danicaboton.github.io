(() => {
  ...
  function refreshRecruitmentAgentLinks(root=document){
    ...
    if(card){
      ...
      const note=card.querySelector('.p01-dual-demo-note');
      const desired='<strong>Custom agent extension</strong>The four-agent architecture remains documented in the case study; the public Project 01 demo currently exposes the Zoho CRM recruiter operating system only.';
      if(note&&note.innerHTML!==desired) note.innerHTML=desired;
    }
