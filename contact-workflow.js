(() => {
  ...
  function useDedicatedMigrationDemo(){document.querySelectorAll('a[href*="demo.html?id=zoho-migration"],iframe[src*="demo.html?id=zoho-migration"],iframe[src*="migration-demo-enterprise.html"],a[href*="migration-demo-enterprise.html"],iframe[src*="migration-demo-control.html"],a[href*="migration-demo-control.html"]').forEach(element=>{const attribute=element.tagName==='IFRAME'?'src':'href';const original=element.getAttribute(attribute)||'';const isEmbed=element.tagName==='IFRAME'||/[?&]embed=1(?:&|$)/.test(original);element.setAttribute(attribute,`migration-demo-control.html?${isEmbed?'embed=1&':''}v=20260905-crashfix2`);});}
