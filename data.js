window.DCODE_PROJECTS = [
  {
    ...
    subtitle: 'A custom Zoho CRM recruitment platform that centralizes applicant operations, automates the recruitment lifecycle through native workflow-triggered Deluge functions, and uses n8n only for resume analysis.',
    ...
    solution: 'I designed a centralized recruitment operating platform in Zoho CRM. Applicants, Employees, Job Openings, Facilities, Workqueue, Tasks, Meetings, Calls, reports, and analytics are connected through CRM-native modules and relationships. Workflow rules trigger Deluge functions that manage record updates, recruitment history, assignments, reporting, validation, and other operational actions inside Zoho CRM. n8n is deliberately isolated to resume processing: it receives an applicant record reference, extracts the resume, compares it with job requirements through structured AI analysis, validates the output, and writes the result back to the Zoho CRM applicant record. No other recruitment automation is delegated to n8n.',
    built: [
    ...
    architecture: ['Zoho CRM Applicant / Job / Facility Records','Native Workflow Rule','Zoho CRM Deluge Function','Record / Subform / Workqueue Updates','Resume Analysis Webhook — only external branch','n8n Resume Extraction + AI Analysis','Validated Zoho CRM Applicant Write-back','Recruiter Ranking View','Reports / Analytics'],
