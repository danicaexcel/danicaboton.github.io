window.DCODE_PROJECTS = [
  {
    ...
    subtitle: 'A custom Zoho CRM recruitment operations platform with native workflow-triggered Deluge automation, recruiter reports, and an n8n workflow used only for resume analysis.',
    ...
    metrics: [
      ['1,432','documented Zoho CRM webhook calls'],
    ...
    problem: 'Recruitment activity was distributed across applicant records, employee records, job openings, facilities, recruiter queues, tasks, meetings, and reports. The team needed one recruitment operating platform inside Zoho CRM, with structured data, reliable stage history, native operational automation, and AI-assisted resume analysis available to recruiters.',
    solution: 'I made Zoho CRM the system of record and the complete recruiter operating surface. Custom modules, fields, layouts, lookups, subforms, reports, workflow rules, and Deluge functions handle the recruitment process natively inside Zoho CRM. n8n has one narrow responsibility: receive the applicant record reference, retrieve the resume and job requirements, run structured AI resume analysis, validate the result, and write the analysis back to Zoho CRM. All other recruitment automation remains inside Zoho CRM.',
    built: [
      'A Zoho CRM recruitment data model covering Applicants, Employees, Job Openings, Facilities, Workqueue, Tasks, Meetings, Calls, Recruitment Report Center, and Recruiter Activity Reports.',
      'Custom modules, fields, layouts, related lists, lookups, and subforms that connect applicant history, employee records, facilities, positions, recruiter ownership, and reporting periods.',
      ...
      'Defensive Deluge functions for stage-history updates, candidate submission, qualification and rejection handling, employee creation or update, validation, recursive-trigger prevention, and post-write verification.',
      'Stage-based recruitment history that preserves prior subform rows and records the recruiter, facility, department, position, employment type, and event date.',
      'Custom fields for recruiter decisions, processing state, experience, education, skills, availability, employment details, and supporting evidence.',
      'Applicant list views where AI Score and Fit Level are visible for recruiter ranking, filtering, and prioritization inside Zoho CRM.',
      'One n8n resume-analysis workflow triggered from Zoho CRM using the Applicant Record ID as the stable retrieval, logging, reprocessing, and write-back key.',
      'Resume attachment lookup, download, file-type detection, text extraction, job-requirement extraction, AI evaluation, structured-output validation, and an explicit error path inside the n8n resume-analysis workflow.',
      ...
      'Model-flexible resume analysis proven with OpenAI GPT-4.1 nano and designed so the model layer can be changed while retaining the same Zoho CRM webhook, validation, and write-back contract.',
      ...
      'A recruitment analytics dashboard available from both Home and Analytics using Zoho CRM-native KPI, comparator, funnel, donut, bar, line, and table components.',
      'A Zoho CRM Workqueue for recruiter activities and personal work queues, plus native report navigation and record-detail views.',
      'Automation evidence modeled on Zoho CRM Setup: workflow rules and schedules use synthetic names and data, while Deluge function analytics and execution-log metrics reproduce non-client operational evidence from surviving production screenshots.'
    ],
    architecture: ['Zoho CRM Applicant Event','Native Workflow Rule','Zoho CRM Deluge Function','Applicant / Employee / Job / Facility Updates','Resume Analysis Webhook','n8n Resume Extraction + AI Analysis','Validated Zoho CRM Write-back','Recruiter Ranking View','Reports / Analytics'],
    reliability: ['Defensive Deluge checks','Record-state validation','Recursive-trigger prevention','Duplicate-processing protection','Structured-output validation','Resume-analysis retry and error path','Post-write verification','Execution history','Failure visibility'],
    outcome: 'The portfolio demo reconstructs the recruiter-facing Zoho CRM environment from surviving interface screenshots, workflow documentation, Deluge execution evidence, n8n resume-analysis runs, and report requirements. Applicants show AI Score and Fit Level for prioritization; record details show structured analysis written back to Zoho CRM; Home and Analytics expose reporting; and Settings demonstrates native workflow rules, schedules, Deluge functions, execution logs, and function analytics. All operational recruitment automation remains inside Zoho CRM, while n8n is limited to resume analysis. Applicant and employee records are synthetic; retained webhook and execution metrics are non-client operational evidence.',
